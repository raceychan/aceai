...
thread 记录。完整 realtime multi-thread runtime 需要 app 层 envelope，例如：
...
TUI、session recorder 和 diagnostics 最终都应消费这个 app-layer envelope。core 仍然只产生
`AgentEvent`，不感知 `thread_id`。
...
### Phase 4: Realtime thread runtime - Not Started
...
- 引入 app-layer `AgentAppEvent(thread_id, agent_id, event)` envelope。
- `delegate_to_subagent` 启动 child `ThreadRuntime`，parent tool call 等待 `handoff_future`。
- child runtime 自己消费 child agent stream，实时记录事件并发给 TUI。
- child suspend / approval / failure / completion 都落在 child runtime 上。
- 如果用户在 child 初始 handoff 前 `/steer`，parent tool call 等待最新有效 run 的 handoff。

验收：

- child 初始运行期间，TUI 能实时看到 child events。
- parent tool call 等待 child handoff，但 child 可被用户切换、观察和 steer。
- child steer 后，parent 收到的是 steer 后最新有效 run 的 handoff。

### Phase 5: Thread-scoped approvals - Not Started
...
- approval-required child tools 可以在 thread runtime 中使用。
- 批准前校验 active thread、run id 和 tool call id 都匹配。

验收：批准 child thread 的工具不会批准 main thread 或其他 child thread。

### Phase 6: Full thread picker and TUI thread state - Not Started

- `/subagents` 从 detail panel 升级为 thread picker。
- 列出 `Main [default] (current)` 和所有 child thread。
- 显示 running / suspended / pending approval / failed / completed 状态。
- 主 stream 显示 active thread。
- topbar/status/input placeholder 显示 active thread label。
- TUI reducer 从单个 `TUIRunState` 升级为 `TUISessionState` / `TUIThreadState`。

验收：用户可以像 Codex `/subagents` 一样在 main 和 child thread 之间浏览、选择和切换。

### Phase 7: Handoff, merge, export, diagnostics - Not Started
