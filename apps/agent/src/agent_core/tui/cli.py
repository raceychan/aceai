...
from agent_core.ace_agent import ACE_AGENT_SKILL_PATH, build_ace_agent
...
from agent_core.provider_auth import default_api_key_for_provider
...
def require_tui_extra() -> None:
    ...
    if SessionStore is not None:
        return
    try:
        session_module = importlib.import_module("agent_core.session")
        replay_module = importlib.import_module("agent_core.tui.session_replay")
        runner_module = importlib.import_module("agent_core.tui.runner")
...
def require_session_extra() -> None:
    ...
    try:
        session_module = importlib.import_module("agent_core.session")
        session_views_module = importlib.import_module("agent_core.session_views")
    except ModuleNotFoundError as exc:
        if exc.name in TUI_RUNTIME_MODULES:
            raise SystemExit(TUI_EXTRA_INSTALL_HINT) from None
        raise
